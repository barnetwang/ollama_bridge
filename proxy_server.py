...
def load_api_usage():
    with api_usage_lock:
        if not os.path.exists(USAGE_FILE):
            usage_data = {"google_search": {"count": 0, "daily_limit": 100, "reset_date": datetime.now().date()}}
            save_api_usage(usage_data)
            return usage_data
        try:
            with open(USAGE_FILE, 'r', encoding='utf-8') as f:
                usage_data = json.load(f)
                usage_data["google_search"]["reset_date"] = datetime.fromisoformat(usage_data["google_search"]["reset_date"]).date()
                return usage_data
...
def call_llm(messages: list, stream: bool = False):
    ...
    try:
        response = requests.post(f"{OLLAMA_BASE_URL}/api/chat", json=payload, stream=stream, timeout=180)
...
def generate_search_query(original_question: str) -> str:
    ...
    payload = {"model": THINKING_MODEL, "prompt": prompt, "stream": False, "options": {"temperature": 0.0}}
    try:
        response = requests.post(f"{OLLAMA_BASE_URL}/api/generate", json=payload, timeout=45)
        response.raise_for_status()
        optimized_query = response.json().get("response", original_question).strip().replace("\"", "")
        logger.info(f"原始問題: '{original_question}' -> 優化後查詢: '{optimized_query}'")
...
def perform_google_search(query: str, max_results: int = 5) -> list[dict]:
    ...
    with api_usage_lock:
        ...
        logger.info(f"Google Search API 使用次數: {API_USAGE['google_search']['count']}/{API_USAGE['google_search']['daily_limit']}")
    ...
    try:
        ...
        if not items:
            ...
            return []
        structured_results = [{"title": item.get('title', 'N/A'), "link": item.get('link', 'N/A'), "snippet": item.get('snippet', 'N/A')} for item in items]
    ...
    for i, result in enumerate(search_results[:3]):
        ...
        try:
            ...
            if not downloaded:
                ...
                continue
            page_main_text = trafilatura.extract(downloaded, include_comments=False, include_tables=True)
            if page_main_text and len(page_main_text) > 100:
                ...
                summary_response = call_llm([{"role": "user", "content": summary_prompt}], stream=False)
                if summary_response:
...
def is_context_relevant(context: str, original_question: str) -> bool:
    ...
    check_prompt = (f"You are a fact-checker... determine if the provided CONTEXT contains enough information to directly answer the USER'S QUESTION... Answer ONLY with 'Yes' or 'No'.\n\n"
                    f"--- USER'S QUESTION ---\n{original_question}\n\n--- CONTEXT ---\n{context}\n\n--- VERDICT (Yes/No) ---")
    payload = {"model": THINKING_MODEL, "prompt": check_prompt, "stream": False, "options": {"temperature": 0.0, "top_p": 0.1}}
    try:
        response = requests.post(f"{OLLAMA_BASE_URL}/api/generate", json=payload, timeout=45)
        response.raise_for_status()
...
def handle_vision_request(adapter, user_prompt, image_base64, final_system_prompt):
    ...
    vision_payload = {"model": VISION_MODEL, "prompt": "Describe this image in detail.", "images": [image_base64], "stream": False}
    try:
    ...
    new_messages = [{"role": "system", "content": f"{final_system_prompt}\n\nImage Description: '{image_description}'."}, {"role": "user", "content": user_prompt}]
    thinking_payload = {"model": THINKING_MODEL, "messages": new_messages, "stream": True}
    try:
        thinking_response = requests.post(f"{OLLAMA_BASE_URL}{adapter.get_final_stream_endpoint()}", json=thinking_payload, stream=True)
...
def intelligent_proxy(subpath):
    ...
    if not (request.method == 'POST' and ("v1/chat/completions" in subpath or "api/chat" in subpath)):
        ...
        try:
            resp = requests.request(method=request.method, url=target_url, headers={k: v for (k, v) in request.headers if k.lower() != 'host'}, data=request.get_data(), params=request.args, stream=True)
    ...
    if not client_request_json.get("stream", False):
        ...
        target_url = f"{OLLAMA_BASE_URL}/{subpath}"
        try:
            resp = requests.post(target_url, headers={k: v for (k, v) in request.headers if k.lower() != 'host'}, json=client_request_json)
    ...
    logger.info(f"--- [STEP 3] 初始解析出的核心問題: '{core_question[:100]}...' ---")
    SEARCH_PREFIX = "@網路搜尋"
    search_context = None
    ...
    if core_question.strip().startswith(SEARCH_PREFIX):
        ...
        if original_question:
            ...
            if search_results:
                search_context = generate_search_context(search_results, original_question)
            image_base64 = None
    ...
    try:
        ...
        parsed_experts = [(p.split('(')[0].strip(), p.split('(')[1].replace(')', '').strip()) for p in parts if '(' in p and ')' in p and p.split('(')[0].strip() in EXPERT_PROMPTS]
        if parsed_experts:
            selected_experts_with_weights = parsed_experts
    except requests.exceptions.RequestException as e:
        logger.warning(f"決策失敗: {e}. 將使用預設專家。", exc_info=True)
    logger.info(f"--- [STEP 5] 模型決策: 選擇專家團隊 -> {selected_experts_with_weights} ---")

    weight_map = {"High": 0, "Medium": 1, "Low": 2}
    sorted_experts = sorted(selected_experts_with_weights, key=lambda x: weight_map.get(x[1], 99))
    ...
    if search_context and original_question:
        if is_context_relevant(search_context, original_question):
            ...
            final_messages.append({"role": "user", "content": original_question})
        else:
        ...
        final_messages.append({"role": "system", "content": base_system_prompt})
        original_messages = client_request_json.get("messages", [])
        final_messages.extend([msg for msg in original_messages if msg.get("role") not in ["system", "developer"]])

    forward_payload = client_request_json.copy()
    forward_payload['messages'] = final_messages
    forward_payload['model'] = THINKING_MODEL
    try:
        ollama_response = requests.post(f"{OLLAMA_BASE_URL}{adapter.get_final_stream_endpoint()}", json=forward_payload, stream=True)
...
if __name__ == '__main__':
    logger.info("="*60)
    logger.info("  Universal Adapter Proxy Started - Refactored & Compatible Version")
